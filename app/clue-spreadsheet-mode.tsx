...
export default function ClueSpreadsheetMode() {
  ...
  const confirmClue = () => {
    try {
      const clean = validateClue(clueText, privateRound?.targetWord ?? null, clueMaxLength);
  ...
  const tableRows = () => {
    if (activeSheet === 'guide') return <>
      ...
      <tr><th>4</th><td>质量评价</td><td colSpan={5}>3 分：准确、有趣并符合本题规则；2 分：有帮助但比较普通；1 分：帮助较少、违背规则或泄露答案。</td></tr>
    ...
    if (activeSheet === 'clues') return <>
      ...
      {room.publicClues.length ? room.publicClues.map((clue, index) => <tr key={clue.clueId}><th>{index + 2}</th><td>{clue.displayCode}</td><td colSpan={2}>{clue.text}</td><td>{clue.roleName ? `${clue.roleName}：${clue.roleRule}` : room.publicRuleName ? `${room.publicRuleName}：${room.publicRuleText}` : '自由提示'}</td><td>{clue.authorName ?? '评分后揭晓'}</td><td>{clue.score ? `${clue.score} 分` : '待评分'}</td></tr>) : <tr><th>2</th><td>尚未汇总</td><td colSpan={5}>所有成员确认或倒计时结束后集中显示。</td></tr>}
    </>;
      ...
      <div className="sheet-grid-scroll"><table className="sheet-grid">
        ...
        <tbody>{activeSheet !== 'home' ? tableRows() : !room ? <>
          ...
          {['guessing', 'rating', 'result'].includes(room.status) && <>
            ...
            {room.publicClues.map((clue, index) => <tr key={clue.clueId}><th>{11 + index}</th><td>{clue.displayCode}</td><td colSpan={2}>{clue.text}</td><td>{clue.roleName ? `${clue.roleName}：${clue.roleRule}` : room.publicRuleName ? `${room.publicRuleName}：${room.publicRuleText}` : '自由提示'}</td><td>{clue.authorName ?? '暂不公开'}</td><td>{room.status === 'rating' && isGuesser ? <select value={ratings[clue.clueId] ?? ''} onChange={(event) => setRatings((current) => ({ ...current, [clue.clueId]: Number(event.target.value) }))}><option value="">评分</option><option value="1">1 分</option><option value="2">2 分</option><option value="3">3 分</option></select> : clue.score ? `${clue.score} 分` : '已公开'}</td></tr>)}
            ...
            {room.status === 'rating' && <><tr><th>25</th><td>评分参考</td><td colSpan={5}>3 分：准确、有趣并符合规则；2 分：有帮助但普通；1 分：帮助较少、违规或泄露答案。</td></tr><tr><th>26</th><td>提示评分</td><td colSpan={4}>{isGuesser ? '请为每条匿名提示选择 1–3 分，确认后揭晓作者。' : `等待 ${room.guesserName} 完成评分。`}</td><td>{isGuesser ? <button className="sheet-action" onClick={confirmRatings}>确认全部评分</button> : '等待中'}</td></tr></>}
