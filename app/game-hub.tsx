...
export default function GameHub() {
  ...
  const [soupRoom, setSoupRoom] = useState<string | null>(null);
  ...
  useEffect(() => {
    window.queueMicrotask(() => {
      ...
      setSoupRoom(savedRoomCode('soup-active-remote'));
    });
  }, []);
  ...
  const rows = useMemo<ReactNode[][]>(() => {
    if (tab === 'guide') return [
      ...
      ['01 选择项目', '在“目录”查看人数、时长和摘要，点击对应行的打开按钮', '所有人', '打开对应页面', 'A2、A3、A4、A5 相互独立', ''],
      ...
      ['', '', '', '', '', ''],
      ['链接规则', '总入口 /', 'A2 /undercover/', 'A3 /clue/', 'A4 /court/', 'A5 /soup/'],
    ];
