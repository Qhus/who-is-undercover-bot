...
import { accuseUndercover, applyBallotResult, autoAdvanceDue, autoVotingDue, blankCardOptions, canAccuseUndercover, canBeginVoting, canTriggerBuzzer, challengeModeLabel, createRoom, dealRoom, descriptionModeLabel, descriptionsAreRevealed, eligibleCandidates, eligibleVoters, exitPlayer, getAccuseUnavailableReason, getBuzzerUnavailableReason, getDescriptionTurnPlayer, getRoundChallenge, getRoundContents, getVotingOpensAt, isRoundContentVisible, joinedPlayerCount, lobbyIsReady, makeId, PLAYER_LIMIT_OPTIONS, PLAYER_NAME_MAX_LENGTH, playingPlayers, revealDescriptions, resolveBallot, resolveUndercoverComeback, ROUND_CONTENT_MAX_LENGTH, selectChallengeRule, setAutoAdvancePaused, setPlayerAway, skipDescription as skipRoundDescription, startDiscussion, startNextRound, startVoting, submitRoundContent as recordRoundContent, triggerBuzzer, undercoverOptions, updateLobbySettings, type ChallengeMode, type DescriptionRevealMode, type GameRoom, type Player } from '@/lib/game';
import { getCloudStore, type GameActionType } from '@/lib/cloudbase-store';
import { randomWordPair, randomWordPairAvoiding, wordPairHint, wordPairKey } from '@/lib/words';
...
function specialActivityCopy(room: GameRoom): string | null {
  if (room.status === 'guessing' && room.pendingComebackPlayerId) {
    const name = playerName(room, room.pendingComebackPlayerId);
    return room.pendingGuessingReason === 'buzzer'
      ? `${name} 发起了猜词爆灯，正在私密输入多数玩家的原词。`
      : `${name} 被投出后正在进行私密反猜。`;
  }
  const result = room.lastCivilianAccuseResult;
  if (result && result.round === room.round && room.status === 'voting') {
    return `${playerName(room, result.accuserId)} 发起了平民爆灯指认，系统已判定 ${playerName(room, result.eliminatedId)} 本轮退出。`;
  }
  return null;
}

function roleRevealCopy(room: GameRoom, playerId: string): string {
  if (room.players.find((player) => player.id === playerId)?.hostOnly) return '出题人 · 未参与本局';
...
function RulesGuide({ onClose }: { onClose: () => void }) {
  ...
  return <div className="rules-guide-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="rules-guide" role="dialog" aria-modal="true" aria-labelledby="rules-guide-title">
      <header><div><span className="stamp">新手先看这里</span><h2 id="rules-guide-title">游玩步骤与完整规则</h2><p>3–10 名实际玩家。系统随机词时房主正常参与；手动填词时房主改为出题人，不参与发牌、描述、投票和胜负。</p></div><button className="rules-guide__close" onClick={onClose} aria-label="关闭游玩步骤与完整规则">关闭 ×</button></header>
      <div className="rules-guide__body">
        <section><span className="panel-kicker">HOW TO PLAY</span><h3>六步开始一局</h3><ol className="play-steps">
          <li><b>创建或加入</b><p>房主设置人数、卧底数、词语和趣味规则并创建房间；其他玩家输入昵称和 6 位房间码加入。手动填词的房主不计入玩家人数。</p></li>
        ...
        <section className="complete-rules"><span className="panel-kicker">FULL RULES</span><h3>完整规则</h3><div className="rule-groups">
          <article><h4>角色与词语</h4><ul><li>3–8 人默认 1 名卧底，9–10 人默认 2 名；3–4 人只允许 1 名。</li><li>平民拿到同一个词，卧底拿到相近但不同的词；空白牌没有具体词语，但会获得一个宽泛的范围提示。</li><li>查看个人信息时只显示词语或空白牌范围提示，不标注角色，身份在本局结束后统一公开。</li></ul></article>
          ...
          <article><h4>特殊操作（可选）</h4><ul><li>猜词爆灯和被投出后的反猜都要求输入多数玩家拿到的完整原词；只有卧底或空白牌答对才能获胜。</li><li>平民爆灯指认中，平民指中特殊阵营则目标退出；指错或特殊阵营发起则发起者退出。</li><li>比对忽略首尾空格、常见标点和英文字母大小写，不接受近义词、简称或意思相近的答案。</li></ul></article>
          <article><h4>公平与隐私</h4><ul><li>秘密词语按住才显示，松开、切走页面或触发遮挡后隐藏；请轮流操作并避免旁观。</li><li>手动填词的房主同时知道两组词，因此只担任出题人；仍可发牌、推进阶段和再开一局。</li><li>这是熟人娱乐工具，不防开发者工具查看数据，也不规避企业网络审计或终端管理。</li><li>若使用自定义词语，房主应选择含义相近、难度相当且不含敏感信息的一对词。</li></ul></article>
        </div></section>
      </div>
...
function Seat({ player }: { player: Player }) {
  return <div className={`seat ${!player.hostOnly && !player.alive ? 'is-out' : player.away ? 'is-away' : ''}`}><span className="seat__number">{player.hostOnly ? '主' : String(player.seat).padStart(2, '0')}</span><span className="seat__avatar">{player.name.slice(0, 1)}</span><span className="seat__name">{player.name}</span><span className="seat__status">{player.hostOnly ? '出题人' : !player.alive ? '已出局' : player.away ? '暂退' : player.cardReady ? '已确认' : '在场'}</span></div>;
}
...
export default function GameApp() {
  ...
  const [specialActionConfirmation, setSpecialActionConfirmation] = useState<SpecialActionConfirmation>(null);
  ...
  const wordReviewPlayer = useMemo(() => {
    ...
    const requested = room.players.find((player) => player.id === wordReviewPlayerId && player.cardReady && !player.hostOnly);
    if (remoteMode) return room.status === 'cards' ? activeCardPlayer : room.players.find((player) => player.id === currentPlayerId && player.cardReady && !player.hostOnly) ?? null;
  ...
  function choosePlayerLimit(limit: number) {
    const undercover = limit >= 9 ? 2 : 1;
    setPlayerLimit(limit);
    setUndercoverCount(undercover);
    if (!blankCardOptions(limit, undercover).includes(blankCardCount)) setBlankCardCount(0);
  }
  function chooseUndercoverCount(count: number) {
    setUndercoverCount(count);
    if (!blankCardOptions(playerLimit, count).includes(blankCardCount)) setBlankCardCount(0);
  }
  function chooseBlankCardCount(count: number) {
    if (!blankCardOptions(playerLimit, undercoverCount).includes(count)) return setNotice({ kind: 'info', text: '当前人数与卧底配置不能再加入空白牌' });
    setBlankCardCount(count);
  }
  function openSetup() { const [civilian, undercover] = randomWordPair(); setCivilianWord(civilian); setUndercoverWord(undercover); setBlankHint(wordPairHint([civilian, undercover])); setScreen('setup'); }
  ...
  function createDemo() {
    ...
    if (blankCardCount > 0 && !blankHint.trim()) return setNotice({ kind: 'error', text: '启用空白牌时需要填写范围提示' });
    const ownerId = makeId('player');
    const created = createRoom({ ownerId, ownerName, playerLimit, undercoverCount, blankCardCount, civilianAccuseEnabled, civilianWord, undercoverWord, blankHint, challengeMode, undercoverComebackEnabled, descriptionRevealMode, buzzerEnabled, autoAdvanceEnabled, wordSource: customWords ? 'manual' : 'random' });
    const base = { ...created, recentWordPairKeys: [wordPairKey([civilianWord, undercoverWord])] };
    const players: Player[] = base.players[0].hostOnly
      ? [base.players[0], ...Array.from({ length: playerLimit }, (_, index) => ({ id: makeId('player'), name: `玩家 ${index + 1}`, seat: index + 1, alive: true, cardReady: false, away: false }))]
      : Array.from({ length: playerLimit }, (_, index) => index === 0 ? base.players[0] : { id: makeId('player'), name: `玩家 ${index + 1}`, seat: index + 1, alive: true, cardReady: false, away: false });
    ...
    setNotice({ kind: 'info', text: base.players[0].hostOnly ? '演示局已创建：房主作为出题人，不参与发牌、描述和投票' : '演示局已创建：这台电脑会依次交给每位玩家操作' });
  }
  ...
  async function createRemote() {
    ...
    try {
      ...
      if (next.players[0]?.hostOnly) setNotice({ kind: 'info', text: `你已作为出题人建房；玩家数量不包含你，请邀请 ${next.playerLimit} 名玩家` });
  ...
  function renamePlayer(id: string, name: string) {
    if (room) setRoom({ ...room, players: room.players.map((player) => player.id === id ? { ...player, name: name.slice(0, PLAYER_NAME_MAX_LENGTH) } : player) });
  }

  function startDealing() {
    if (!room) return;
    if (room.players.some((player) => !player.name.trim())) return setNotice({ kind: 'error', text: '每个座位都需要一个名字' });
    try { const next = dealRoom(room); commitRoom(next); setRevealPlayerId(eligibleVoters(next)[0]?.id ?? null); setPrivacyGate(true); }
  ...
  function requestSpecialAction(type: 'buzzer' | 'accuse', playerId: string) {
    if (!room) return;
    const reason = type === 'buzzer'
      ? getBuzzerUnavailableReason(room, playerId, clockNow)
      : getAccuseUnavailableReason(room, playerId, clockNow);
    if (reason) return setNotice({ kind: 'info', text: reason });
    setSpecialActionConfirmation({ type, playerId });
  }

  function confirmSpecialAction() {
    const pending = specialActionConfirmation;
    if (!pending) return;
    setSpecialActionConfirmation(null);
    if (pending.type === 'buzzer') startBuzzer(pending.playerId);
    else openCivilianAccuse(pending.playerId);
  }
  ...
  function rematch() {
    ...
    const blankHint = wordPairHint([civilianWord, undercoverWord]);
    const recentWordPairKeys = [...recent, wordPairKey([civilianWord, undercoverWord])].slice(-10);
    const fresh = dealRoom({ ...room, civilianWord, undercoverWord, blankHint, recentWordPairKeys, status: 'lobby', players: room.players.map((player) => player.hostOnly ? { ...player, alive: false, away: false, cardReady: true } : { ...player, alive: true, away: false, cardReady: false }) });
    commitRoom(fresh);
    setRevealPlayerId(eligibleVoters(fresh)[0]?.id ?? null);
    setPrivacyGate(true);
  }
  ...
  async function copyCurrentRule() { if (!room) return; const text = `本局设置：${challengeModeLabel(room.challengeMode ?? 'off')}挑战｜描述方式：${descriptionModeLabel(room.descriptionRevealMode ?? 'all_submitted')}｜猜词翻盘${room.undercoverComebackEnabled ? '开启' : '关闭'}｜猜词爆灯${room.buzzerEnabled ? '开启' : '关闭'}｜自动下一轮${(room.autoAdvanceEnabled ?? true) ? '开启' : '关闭'}\n房主身份：${room.wordSource === 'manual' ? '出题人，不参与发牌、描述和投票' : '普通玩家'}\nRound_${String(room.round).padStart(2, '0')}：${getRoundChallenge(room, room.round)?.text ?? '无附加规则'}\n挑战规则由玩家自觉遵守。`; try { await navigator.clipboard.writeText(text); } catch { /* clipboard may be unavailable */ } setNotice({ kind: 'info', text: '本轮公共规则已复制' }); }
  function immersiveSpecialActions() {
    if (!room || (room.status !== 'discussion' && room.status !== 'voting')) return null;
    const players = remoteMode ? room.players.filter((player) => player.id === currentPlayerId && !player.hostOnly) : room.players.filter((player) => player.alive && !player.hostOnly);
    if (!room.buzzerEnabled && !room.civilianAccuseEnabled) return null;
    return <div className="buzzer-actions"><span>特殊操作</span>{players.map((player) => {
      const buzzerReason = getBuzzerUnavailableReason(room, player.id, clockNow);
      const accuseReason = getAccuseUnavailableReason(room, player.id, clockNow);
      return <span key={player.id}>{room.buzzerEnabled && <button disabled={Boolean(buzzerReason)} title={buzzerReason ?? '可发起'} onClick={() => requestSpecialAction('buzzer', player.id)}>{remoteMode ? '我要猜词爆灯' : `${player.name} 猜词爆灯`}{buzzerReason ? ` · ${buzzerReason}` : ''}</button>}{room.civilianAccuseEnabled && <button disabled={Boolean(accuseReason)} title={accuseReason ?? '可发起'} onClick={() => requestSpecialAction('accuse', player.id)}>{remoteMode ? '我要平民爆灯指认' : `${player.name} 平民爆灯指认`}{accuseReason ? ` · ${accuseReason}` : ''}</button>}</span>;
    })}</div>;
  }
  function reset() { setScreen('home'); setRoom(null); setRemoteMode(false); setCurrentPlayerId(null); setRevealPlayerId(null); setVotePlayerId(null); setDiscussionPlayerId(null); setRoundContentDraft(''); setComebackDraft(''); setWordReviewPlayerId(null); setAccuseActorId(null); setAccuseTargetId(null); setSpecialActionConfirmation(null); setPrivacyGate(true); window.localStorage.removeItem('undercover-demo-room'); window.localStorage.removeItem('undercover-active-remote'); }
  ...
  if (displayMode === 'spreadsheet') return <SpreadsheetMode
    ...
    onBeginVoting={beginVoting} onSkipDescription={skipCurrentDescription} onBuzzer={(playerId) => requestSpecialAction('buzzer', playerId)} onSubmitVote={submitVote} onComebackDraft={setComebackDraft} onSubmitComeback={submitComeback} onContinue={continueGame} onToggleAutoAdvance={toggleAutoAdvance} onRematch={rematch} onOwnerName={setOwnerName}
    blankCardCount={blankCardCount} civilianAccuseEnabled={civilianAccuseEnabled} lobbySettingsDraft={lobbySettingsDraft} accuseActorId={accuseActorId} accuseTargetId={accuseTargetId} specialActionConfirmation={specialActionConfirmation}
    blankHint={blankHint} onBlankHint={setBlankHint} onLobbySettingsDraft={setLobbySettingsDraft} onSaveLobbySettings={saveLobbySettings} onOpenCivilianAccuse={(playerId) => requestSpecialAction('accuse', playerId)} onAccuseTarget={setAccuseTargetId} onCancelCivilianAccuse={cancelCivilianAccuse} onSubmitCivilianAccuse={submitCivilianAccuse} onConfirmSpecialAction={confirmSpecialAction} onCancelSpecialAction={() => setSpecialActionConfirmation(null)}
    onPlayerLimit={choosePlayerLimit} onUndercoverCount={chooseUndercoverCount} onBlankCardCount={chooseBlankCardCount} onCivilianAccuseEnabled={setCivilianAccuseEnabled} onCivilianWord={(value) => { setCivilianWord(value); setCustomWords(true); setBlankHint(''); }}
    onUndercoverWord={(value) => { setUndercoverWord(value); setCustomWords(true); setBlankHint(''); }} onRandomWords={() => { const [a, b] = randomWordPair(); setCivilianWord(a); setUndercoverWord(b); setBlankHint(wordPairHint([a, b])); setCustomWords(false); }}
    onCustomWords={() => { setCustomWords(true); setCivilianWord(''); setUndercoverWord(''); setBlankHint(''); }} onChallengeMode={setChallengeMode} onUndercoverComebackEnabled={setUndercoverComebackEnabled} onDescriptionRevealMode={setDescriptionRevealMode} onBuzzerEnabled={setBuzzerEnabled} onAutoAdvanceEnabled={setAutoAdvanceEnabled} onJoinCode={setJoinCode} onJoinName={setJoinName}
  ...
  return <main className="app-shell">
    ...
    {screen === 'setup' && <div className="workspace setup-page">
      ...
      <div className="setup-grid"><section className="paper-card">
        <div className="field-row"><div><label htmlFor="owner-name">{customWords ? '出题人称呼' : '你的称呼'}</label><p>{customWords ? '你会保留房主权限，但不参与游戏' : '你会成为这局的房主并正常参与'}，最多 {PLAYER_NAME_MAX_LENGTH} 字</p></div><input id="owner-name" value={ownerName} maxLength={PLAYER_NAME_MAX_LENGTH} onChange={(event) => setOwnerName(event.target.value.slice(0, PLAYER_NAME_MAX_LENGTH))} /></div>
        <div className="field-row field-row--stack"><div><label>玩家人数</label><p>可设置 3–10 人；{customWords ? '不包含出题房主' : '包含参与游戏的房主'}</p></div><div className="segmented segmented--many">{PLAYER_LIMIT_OPTIONS.map((limit) => <button className={playerLimit === limit ? 'is-selected' : ''} onClick={() => choosePlayerLimit(limit)} key={limit}>{limit} 人</button>)}</div></div>
        <div className="field-row field-row--stack"><div><label>卧底人数</label><p>3–8 人默认 1 名，9–10 人默认 2 名</p></div><div className="segmented">{undercoverOptions(playerLimit, blankCardCount).map((count) => <button className={undercoverCount === count ? 'is-selected' : ''} onClick={() => chooseUndercoverCount(count)} key={count}>{count} 名</button>)}</div></div>
        <div className="field-row field-row--stack"><div><label>空白牌人数</label><p>启用后获得范围提示，不会拿到具体词语</p></div><div className="segmented">{blankCardOptions(playerLimit, undercoverCount).map((count) => <button className={blankCardCount === count ? 'is-selected' : ''} onClick={() => chooseBlankCardCount(count)} key={count}>{count} 张</button>)}</div></div>
        ...
        <div className="field-row field-row--stack"><div><label>平民爆灯指认</label><p>描述公开后每局一次；指错或特殊阵营发起则本人退出</p></div><div className="segmented"><button className={!civilianAccuseEnabled ? 'is-selected' : ''} onClick={() => setCivilianAccuseEnabled(false)}>关闭</button><button className={civilianAccuseEnabled ? 'is-selected' : ''} onClick={() => setCivilianAccuseEnabled(true)}>开启</button></div></div>
        <div className="field-row field-row--stack"><div><label>自动进入下一轮</label><p>结果展示 7 秒后自动继续，房主可以暂停</p></div><div className="segmented"><button className={!autoAdvanceEnabled ? 'is-selected' : ''} onClick={() => setAutoAdvanceEnabled(false)}>关闭</button><button className={autoAdvanceEnabled ? 'is-selected' : ''} onClick={() => setAutoAdvanceEnabled(true)}>开启</button></div></div>
        <div className="field-row field-row--stack"><div><label>词语来源</label><p>{customWords ? '手动填词时，房主是出题人，不计入玩家人数' : '系统随机时，房主作为普通玩家参与'}</p></div><div className="segmented"><button className={!customWords ? 'is-selected' : ''} onClick={() => { const [a, b] = randomWordPair(); setCivilianWord(a); setUndercoverWord(b); setBlankHint(wordPairHint([a, b])); setCustomWords(false); }}>系统随机</button><button className={customWords ? 'is-selected' : ''} onClick={() => { setCustomWords(true); setCivilianWord(''); setUndercoverWord(''); setBlankHint(''); }}>自定义</button></div></div>
        {customWords ? <div className="word-grid"><label>词语 A<input value={civilianWord} onChange={(event) => { setCivilianWord(event.target.value); setCustomWords(true); setBlankHint(''); }} placeholder="输入词语 A" aria-label="普通成员词语" /></label><label>词语 B<input value={undercoverWord} onChange={(event) => { setUndercoverWord(event.target.value); setCustomWords(true); setBlankHint(''); }} placeholder="输入相近的词语 B" aria-label="卧底成员词语" /></label></div> : <div className="setup-hint" role="status">两组词会由系统生成，创建前不展示；房主可正常参与。</div>}
        {blankCardCount > 0 && <div className="field-row"><div><label htmlFor="blank-hint">空白牌范围提示</label><p>只让空白牌看到宽泛类别，不填写具体答案</p></div><input id="blank-hint" value={blankHint} maxLength={30} onChange={(event) => setBlankHint(event.target.value.slice(0, 30))} placeholder="例如：工作或学习场景" /></div>}
    ...
    {screen === 'game' && room && <div className="workspace game-page"><Progress room={room} /><div className="game-heading"><div><p className="eyebrow">{statusCopy(room)}</p><h1>{room.status === 'lobby' ? '确认玩家名单' : room.status === 'cards' ? '把电脑交给指定玩家' : room.status === 'discussion' ? '按本轮规则来描述。' : room.status === 'voting' ? (room.ballot === 2 ? '平票了，只投并列者。' : '请依次秘密投票。') : room.status === 'guessing' ? '正在进行特殊判定。' : room.status === 'finished' ? (room.winner === 'undercover' ? '流程已完成。' : '胜负已定。') : '这一轮，有结果了。'}</h1></div><div className="round-badge"><span>ROUND</span><b>{String(room.round).padStart(2, '0')}</b></div></div>
      ...
      {specialActivityCopy(room) && <div className="waiting-line" role="status">全体提示：{specialActivityCopy(room)}</div>}
      {specialActionConfirmation && <section className="waiting-panel"><span className="stamp">操作确认</span><h2>{specialActionConfirmation.type === 'buzzer' ? '猜词爆灯' : '平民爆灯指认'}</h2><p>{specialActionConfirmation.type === 'buzzer' ? '请输入你认为多数玩家拿到的完整原词。只有卧底或空白牌答对才获胜；身份不符、答案不一致或超时都会退出。' : '平民指中特殊阵营则目标退出；指错或由特殊阵营发起，发起者退出。'}</p><div className="result-actions"><button className="button button--primary" onClick={confirmSpecialAction}>我已了解，继续</button><button className="button button--outline" onClick={() => setSpecialActionConfirmation(null)}>取消</button></div></section>}
      {accuseActorId && canAccuseUndercover(room, accuseActorId, clockNow) && (!remoteMode || accuseActorId === currentPlayerId) && <section className="voting-descriptions"><div><span className="panel-kicker">平民爆灯指认</span><h2>选择你要指认的成员</h2><p>确认后不可撤销；指中卧底或空白牌则目标退出，指错则发起者退出。</p></div><div className="candidate-grid">{eligibleVoters(room).filter((candidate) => candidate.id !== accuseActorId).map((candidate) => <button className={accuseTargetId === candidate.id ? 'is-selected' : ''} onClick={() => setAccuseTargetId(candidate.id)} key={candidate.id}><span>{candidate.name.slice(0, 1)}</span><b>{candidate.name}</b><i>{accuseTargetId === candidate.id ? '已选择' : '选择'}</i></button>)}</div><div className="result-actions"><button className="button button--primary" disabled={!accuseTargetId} onClick={submitCivilianAccuse}>确认指认</button><button className="button button--outline" onClick={cancelCivilianAccuse}>取消</button></div></section>}
      ...
      {(['discussion', 'voting', 'result'] as const).includes(room.status as 'discussion' | 'voting' | 'result') && !wordReviewPlayerId && (remoteMode ? wordReviewPlayer : room.players.some((player) => player.cardReady && !player.hostOnly)) && <div className="word-review-actions">{remoteMode && wordReviewPlayer ? <button onClick={() => { setWordReviewPlayerId(wordReviewPlayer.id); setPrivacyGate(true); setRevealed(false); }}>再次查看自己的词语</button> : room.players.filter((player) => player.cardReady && !player.hostOnly).map((player) => <button key={player.id} onClick={() => { setWordReviewPlayerId(player.id); setPrivacyGate(true); setRevealed(false); }}>{player.name} · 复看词语</button>)}</div>}
      {wordReviewPlayerId && wordReviewPlayer && (['discussion', 'voting', 'result'] as const).includes(room.status as 'discussion' | 'voting' | 'result') && <section className="private-word-review">{privacyGate ? <div className="privacy-gate"><span className="seat__avatar seat__avatar--large">{wordReviewPlayer.name.slice(0, 1)}</span><p>再次查看词语</p><h2>{wordReviewPlayer.name}</h2><span>请确认屏幕前只有你本人</span><button className="button button--dark" onClick={() => setPrivacyGate(false)}>我就是本人</button><button className="back-link" onClick={() => setWordReviewPlayerId(null)}>取消</button></div> : <div className={`identity-card ${revealed ? 'is-revealed' : ''}`}><div className="identity-card__cover"><span>按住鼠标或空格键</span><b>查看自己的词语</b><i>松手立即遮挡</i></div><div className="identity-card__secret"><span>仅你可见</span><p>自己的词语</p><strong>{room.assignments[wordReviewPlayer.id]?.role === 'blank' ? blankCardCopy(room) : room.assignments[wordReviewPlayer.id]?.word}</strong></div><button aria-label="按住再次查看自己的秘密词语" onPointerDown={() => setRevealed(true)} onPointerUp={() => setRevealed(false)} onPointerLeave={() => setRevealed(false)} onKeyDown={(event) => { if (event.code === 'Space') { event.preventDefault(); setRevealed(true); } }} onKeyUp={(event) => { if (event.code === 'Space') setRevealed(false); }} /><button className="word-review-close" onClick={() => { setRevealed(false); setWordReviewPlayerId(null); }}>关闭复看</button></div>}</section>}
      {room.status === 'lobby' && <section className="game-card lobby-card"><div className="section-title"><div><span className="panel-kicker">座位表</span><h2>{joinedPlayerCount(room)}/{room.playerLimit} 名玩家已就位</h2></div><span>{room.players.some((player) => player.hostOnly) ? '房主作为出题人，不计入玩家人数' : remoteMode ? '分享房间码邀请朋友' : '可直接改名'}</span></div><div className="lobby-list">{room.players.map((player) => <label className="lobby-player" key={player.id}><span>{player.hostOnly ? '主' : String(player.seat).padStart(2, '0')}</span><input disabled={remoteMode} value={player.name} onChange={(event) => renamePlayer(player.id, event.target.value)} /><i>{player.hostOnly ? '出题人' : player.id === room.ownerId ? '房主玩家' : '玩家'}</i></label>)}</div>{!remoteMode || currentPlayerId === room.ownerId ? <button className="button button--primary button--wide" disabled={!lobbyIsReady(room)} onClick={startDealing}>{lobbyIsReady(room) ? '锁定名单并随机发牌' : `还差 ${room.playerLimit - joinedPlayerCount(room)} 名玩家`} <span>→</span></button> : <div className="waiting-line">等待房主在玩家到齐后发牌…</div>}</section>}
      {room.status === 'cards' && activeCardPlayer && <section className="private-stage"><aside className="player-queue"><span className="panel-kicker">个人信息进度</span><h2>{playingPlayers(room).filter((player) => player.cardReady).length}/{playingPlayers(room).length} 已确认</h2>{room.players.map((player) => <Seat player={player} key={player.id} />)}</aside><div className="private-card-wrap">{privacyGate ? <div className="privacy-gate"><span className="seat__avatar seat__avatar--large">{activeCardPlayer.name.slice(0, 1)}</span><p>下一位</p><h2>{activeCardPlayer.name}</h2><span>请确认身边没有人偷看屏幕</span><button className="button button--dark" onClick={() => setPrivacyGate(false)}>我就是本人</button></div> : <div className={`identity-card ${revealed ? 'is-revealed' : ''}`}><div className="identity-card__cover" aria-hidden={revealed}><span>按住鼠标或空格键</span><b>查看我的词语</b><i>松手立即遮挡</i></div><div className="identity-card__secret" aria-hidden={!revealed}><span>仅你可见</span><p>你的词语</p><strong>{room.assignments[activeCardPlayer.id].role === 'blank' ? blankCardCopy(room) : room.assignments[activeCardPlayer.id].word}</strong></div><button aria-label="按住查看自己的秘密词语，不显示角色" onPointerDown={() => setRevealed(true)} onPointerUp={() => setRevealed(false)} onPointerLeave={() => setRevealed(false)} onKeyDown={(event) => { if (event.code === 'Space') { event.preventDefault(); setRevealed(true); } }} onKeyUp={(event) => { if (event.code === 'Space') setRevealed(false); }} /></div>}{!privacyGate && <button className="button button--primary button--wide" onClick={confirmCard} disabled={revealed}>已确认自己的词语 <span>→</span></button>}</div></section>}
      {room.status === 'cards' && remoteMode && !activeCardPlayer && <section className="waiting-panel"><span className="stamp">{room.players.find((player) => player.id === currentPlayerId)?.hostOnly ? '出题人工作台' : '私牌已确认'}</span><h2>{room.players.find((player) => player.id === currentPlayerId)?.hostOnly ? '等待玩家确认词牌。' : '请把注意力放回桌边。'}</h2><p>还有 {eligibleVoters(room).filter((player) => !player.cardReady).length} 位在场玩家没有确认私牌；全部完成后会自动进入讨论。</p></section>}
      {room.status === 'discussion' && <section className="discussion-card">
        ...
        <div>
          ...
          {activeDiscussionPlayer && !descriptionsAreRevealed(room, clockNow) && <div className="round-content-form"><label htmlFor="round-content">{activeDiscussionPlayer.name} 的本轮内容 · {roundContentDraft.length} 字</label><div><input id="round-content" maxLength={ROUND_CONTENT_MAX_LENGTH} value={roundContentDraft} onChange={(event) => setRoundContentDraft(event.target.value)} placeholder={`填写本轮描述｜规则：${getRoundChallenge(room, room.round)?.text ?? '自由表达'}`} /><button className="button button--dark" disabled={!roundContentDraft.trim()} onClick={submitCurrentRoundContent}>提交本轮内容</button></div></div>}
          ...
          {immersiveSpecialActions()}
        </div>
      </section>}
      {room.status === 'voting' && <section className="voting-descriptions"><div><span className="panel-kicker">投票参考</span><h2>本轮所有公开描述</h2></div><div className="description-review">{room.players.filter((player) => player.alive || getRoundContents(room)[player.id]).map((player) => <div key={player.id}><b>{player.name}</b><span>{getRoundContents(room)[player.id] ?? '本轮未提交'}</span></div>)}</div>{immersiveSpecialActions()}</section>}
      ...
      {room.status === 'guessing' && activeComebackPlayer && <section className="comeback-panel">{privacyGate ? <div className="privacy-gate"><span className="seat__avatar seat__avatar--large">{activeComebackPlayer.name.slice(0, 1)}</span><p>{room.pendingGuessingReason === 'buzzer' ? '猜词爆灯' : '私密反猜'}</p><h2>{activeComebackPlayer.name}</h2><span>请输入你认为多数玩家拿到的完整原词；答案不一致或超时会退出。倒计时正在继续。</span><button className="button button--dark" onClick={() => setPrivacyGate(false)}>我已了解，开始反猜</button></div> : <div className="comeback-form"><span className="stamp">{room.pendingGuessingReason === 'buzzer' ? '爆灯只能尝试一次' : '全阵营仅此一次'}</span><h2>猜出多数玩家的原词</h2><strong>{formatCountdown(comebackRemainingSeconds)}</strong><p>{room.pendingGuessingReason === 'buzzer' ? '只有卧底或空白牌输入正确的平民原词才能获胜；身份不符、答案不一致或超时都会立即退出。' : '只能提交一次；输入正确的平民原词后特殊阵营立即获胜，答案不一致或超时则正常退出。'}</p><input value={comebackDraft} onChange={(event) => setComebackDraft(event.target.value.slice(0, 30))} placeholder="输入多数玩家拿到的完整原词" aria-label="输入多数玩家拿到的完整原词" autoFocus /><button className="button button--primary button--wide" disabled={!comebackDraft.trim()} onClick={submitComeback}>确认提交答案 <span>→</span></button></div>}</section>}
      {room.status === 'guessing' && !activeComebackPlayer && <section className="waiting-panel"><span className="stamp">{room.pendingGuessingReason === 'buzzer' ? '猜词爆灯' : '私密反猜'}</span><h2>{formatCountdown(comebackRemainingSeconds)}</h2><p>{specialActivityCopy(room)}结果提交或倒计时结束后会统一处理。</p></section>}
