...
export default function GameApp() {
  ...
  function createDemo() {
    ...
    const created = createRoom({ ownerId, ownerName, playerLimit, undercoverCount, blankCardCount, civilianAccuseEnabled, civilianWord, undercoverWord, blankHint, challengeMode, undercoverComebackEnabled, descriptionRevealMode, buzzerEnabled, autoAdvanceEnabled });
  ...
  function rematch() {
    ...
    const fresh = dealRoom({ ...room, civilianWord, undercoverWord, blankHint, recentWordPairKeys, status: 'lobby', players: room.players.map((player) => ({ ...player, alive: true, away: false, cardReady: false })) });
    commitRoom(fresh);
  ...
  function immersiveSpecialActions() {
    ...
    const players = remoteMode ? room.players.filter((player) => player.id === currentPlayerId) : room.players.filter((player) => player.alive);
  ...
  return <main className="app-shell">
    ...
    {screen === 'setup' && <div className="workspace setup-page">
      ...
      <div className="setup-grid"><section className="paper-card">
        <div className="field-row"><div><label htmlFor="owner-name">你的称呼</label><p>你会成为这局的房主，最多 {PLAYER_NAME_MAX_LENGTH} 字</p></div><input id="owner-name" value={ownerName} maxLength={PLAYER_NAME_MAX_LENGTH} onChange={(event) => setOwnerName(event.target.value.slice(0, PLAYER_NAME_MAX_LENGTH))} /></div>
