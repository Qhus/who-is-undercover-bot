...
with checks(object_type, object_name, expected, actual) as (
  values
    ...
    ('table', 'public.game_actions', 'exists', case when to_regclass('public.game_actions') is null then 'missing' else 'exists' end),
    ...
    ('function', 'public.apply_game_action(text,text,text,text,integer,integer,bigint,jsonb)', 'exists', case when to_regprocedure('public.apply_game_action(text,text,text,text,integer,integer,bigint,jsonb)') is null then 'missing' else 'exists' end),
    ...
    ('rls', 'public.game_actions', 'enabled', coalesce((select case when relrowsecurity then 'enabled' else 'disabled' end from pg_class where oid = to_regclass('public.game_actions')), 'missing')),
    ...
    ('policy', 'game_actions_member_select', 'exists', case when exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'game_actions' and policyname = 'game_actions_member_select') then 'exists' else 'missing' end)
)
