-- LFR-blank/civilian-accuse：在 concurrency-v2.sql 基础上增加特殊角色并发操作。
-- 执行顺序：先执行 concurrency-v2.sql，再执行本文件；本文件不会删除或覆盖 v2 的表与策略。
-- 本文件只准备迁移，不在本地执行。完成双浏览器验收后再由管理员在 CloudBase 控制台执行。

-- 保留 v2 实现，新的入口函数只拦截本迁移新增的两类操作，其余操作原样委托给 v2。
alter function public.apply_game_action(text, text, text, text, integer, integer, bigint, jsonb)
  rename to apply_game_action_v2;
...
declare
  ...
  v_accuse_enabled boolean;
begin
  if p_action_type not in ('update_lobby_settings', 'accuse_undercover') then
    return public.apply_game_action_v2(p_code, p_action_id, p_action_type, p_expected_status, p_expected_round, p_expected_ballot, p_expected_version, p_payload);
  end if;
  ...
  if found then return jsonb_build_object('outcome', 'duplicate', 'code', 'ALREADY_APPLIED', 'message', '操作已经完成', 'state', v_state, 'version', v_game.version); end if;
  ...
  if p_action_type = 'update_lobby_settings' then
    ...
    v_accuse_enabled := coalesce((p_payload->>'civilianAccuseEnabled')::boolean, false);
    ...
    v_state := jsonb_set(v_state, '{civilianAccuseEnabled}', to_jsonb(v_accuse_enabled), true);
  else
    if v_state->>'status' <> 'voting' or v_state->>'descriptionsRevealedAt' is null then
    ...
    v_target_id := p_payload->>'targetId';
    ...
    select jsonb_agg(case when value->>'id' = v_eliminated_id then jsonb_set(jsonb_set(value, '{alive}', 'false'::jsonb, true), '{away}', 'false'::jsonb, true) else value end order by (value->>'seat')::integer) into v_players from jsonb_array_elements(v_state->'players');
    v_state := jsonb_set(v_state, '{players}', v_players, true);
    select count(*) filter (where coalesce((p->>'alive')::boolean, false) and v_state->'assignments'->(p->>'id')->>'role' in ('undercover','blank')), count(*) filter (where coalesce((p->>'alive')::boolean, false) and v_state->'assignments'->(p->>'id')->>'role' not in ('undercover','blank')) into v_special, v_civilians from jsonb_array_elements(v_players) p;
    ...
    if v_winner is not null then v_state := jsonb_set(v_state, '{lastResult}', jsonb_build_object('round', (v_state->>'round')::integer, 'ballot', 1, 'counts', '{}'::jsonb, 'tiedIds', '[]'::jsonb, 'eliminatedId', v_eliminated_id, 'noElimination', false), true); end if;
  end if;
...
revoke all on function public.apply_game_action_v2(text, text, text, text, integer, integer, bigint, jsonb) from public;
