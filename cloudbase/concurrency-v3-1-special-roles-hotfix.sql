-- V3.1 特殊角色热修复：在已经执行 concurrency-v3-special-roles.sql 的环境运行。
-- 需要使用 postgres 或当前 apply_game_action 函数所有者执行。
-- 本文件使用 create or replace，不再次重命名函数，可安全重复执行。
...
declare
  ...
  v_delay integer;
begin
  -- V2 的投票结算不认识空白牌。先复用其锁、幂等和计票，再修正特殊阵营结算。
  if p_action_type = 'submit_vote' then
    v_delegate := public.apply_game_action_v2(p_code, p_action_id, p_action_type, p_expected_status, p_expected_round, p_expected_ballot, p_expected_version, p_payload);
