...
export interface SoupDraftSnapshot { text: string; solutionText: string; status: SoupDraftStatus; }
...
export function canEditSoupDraft(room: SoupRoom, playerId: string) {
  return room.round > 0 && room.hostId !== playerId && room.players.some((p) => p.id === playerId)
    && !['lobby', 'round_result', 'feedback', 'finished'].includes(room.status);
}
...
export function createSoupDraftController(options: {
  save: (text: string, solutionText: string, revision: number) => Promise<SoupDraftSaveResult>;
  ...
  const emit = (status: SoupDraftStatus) => { if (!disposed) options.onChange({ text, solutionText, status }); };
  const cache = () => options.cache(text === savedText && solutionText === savedSolutionText ? null : JSON.stringify({ question: text, solution: solutionText }));

  const flush = async (): Promise<void> => {
    if (disposed || !enabled || !ready || saving || conflict || (text === savedText && solutionText === savedSolutionText)) return;
    saving = true;
    emit('saving');
    const sent = text;
    const sentSolution = solutionText;
    try {
      const result = await options.save(sent, sentSolution, revision);
      if (disposed) return;
      revision = result.privateRound.draftRevision ?? 0;
      if (!result.accepted && (result.privateRound.draftText !== sent || (result.privateRound.solutionDraft ?? '') !== sentSolution)) {
        conflict = result.privateRound;
        emit('conflict');
        return;
      }
      savedText = sent;
      savedSolutionText = sentSolution;
      cache();
      emit(text === savedText && solutionText === savedSolutionText ? 'saved' : 'idle');
    } catch {
      if (!disposed) { cache(); emit('error'); }
      return;
    } finally { saving = false; }
    if (!disposed && (text !== savedText || solutionText !== savedSolutionText)) await flush();
  };
  ...
  return {
    hydrate(packet: SoupPrivateRound, localText: string | null = null) {
      ...
      savedSolutionText = packet.solutionDraft ?? '';
      let local: { question: string; solution: string } | null = null;
      if (localText !== null) {
        try { local = JSON.parse(localText) as { question: string; solution: string }; }
        catch { local = { question: localText, solution: '' }; }
      }
      text = local?.question ?? savedText;
      solutionText = local?.solution ?? savedSolutionText;
      // A restored unsaved draft is never silently written over a newer device's draft.
      if (local !== null && (text !== savedText || solutionText !== savedSolutionText)) conflict = packet;
    ...
    resolveConflict(useLocal: boolean) {
      ...
      savedSolutionText = conflict.solutionDraft ?? '';
      revision = conflict.draftRevision ?? 0;
      if (!useLocal) { text = savedText; solutionText = savedSolutionText; }
      conflict = null;
      cache();
      emit(text === savedText && solutionText === savedSolutionText ? 'saved' : 'idle');
